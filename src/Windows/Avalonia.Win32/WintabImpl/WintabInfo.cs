...
namespace Avalonia.Win32.WintabImpl
{
    ...
    public class WintabInfo
    {
        ...
        /*
         TODO: Likely wrong?
        public static void GetTabletDimensions(out int maxX, out int maxY, out int maxPressure)
        {
            int size = Marshal.SizeOf<WintabAxis>();
            IntPtr ptr = Marshal.AllocHGlobal(size);

            // Get X Axis info
            // WTI_DEVICES = 100, DVC_X = 12
            WintabFuncs.WTInfoA(100, 12, ptr);
            var xAxis = Marshal.PtrToStructure<WintabAxis>(ptr);
            maxX = xAxis.axMax;

            // Get Y Axis info
            // DVC_Y = 13
            WintabFuncs.WTInfoA(100, 13, ptr);
            var yAxis = Marshal.PtrToStructure<WintabAxis>(ptr);
            maxY = yAxis.axMax;

            // Get Pressure info
            // DVC_NPRESSURE = 15
            WintabFuncs.WTInfoA(100, 15, ptr);
            var pAxis = Marshal.PtrToStructure<WintabAxis>(ptr);
            maxPressure = pAxis.axMax;
